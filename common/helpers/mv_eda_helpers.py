...
def markval_by_time(df, time_var, summarizer, facet_var, legend = True, group_var='main_field_position'):
  if(summarizer == "mean"):
    plot_df = ( df >> 
...
def scatter_mv_change(df,xvar,yvar,facet_var):
  if(facet_var!='none'):
    ...
    fig = px.scatter(df, xvar, yvar, hover_data = ["name","age","nationality","club"],
                    trendline="ols", color = "main_field_position")
    fig.for_each_annotation(lambda a: a.update(text=a.text.split("=")[1]))
    return fig